...
from mmcv_custom import my_load_checkpoint as load_checkpoint      #原始的
# from mmcv.runner import load_checkpoint                          #跑SAM自己改的
...
class TIMMVisionTransformer(BaseModule):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000,
        ...
        all_keys = self.init_weights(pretrained)
        
        # """冻结参数"""
        # sum=0
        # for name, param in self.named_parameters():
        #     # print("{:30s} : {},{}".format(name,param.shape,param.requires_grad)) 
        #     if name in all_keys:
    ...
    def init_weights(self, pretrained=None):
        if isinstance(pretrained, str):
            ...
            checkpoint, all_keys = load_checkpoint(self, pretrained, map_location='cpu', strict=False, logger=logger)  #原始的
            # checkpoint, all_keys = load_checkpoint(self, pretrained, map_location='cpu', strict=False, logger=logger, revise_keys=[(r'^image_encoder\.', ''), ('lin', 'fc')]) #跑SAM改动过的
            return all_keys
