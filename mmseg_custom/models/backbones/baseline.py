...
class UniBaseline(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 embed_layer=VisualPatchEmbedding, window_attn=False, window_size=14, with_cp=False,
                 output_dim=512, out_indices=[3, 5, 7, 11], pretrained=None):
        
        super(UniBaseline, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        if isinstance(pretrained, str):
            ...
            checkpoint, all_keys = load_checkpoint(self, pretrained, map_location='cpu', strict=False, logger=logger, revise_keys=[(r'^backbone\.', '')])
            return all_keys
