...
class Mask2FormerHead(BaseDecodeHead):
    ...
    图像分割中的分割对象分为things和stuff两大类。things是可数目标,包括人、动物、汽车等。而stuff是不可数目标,包括草地、天空、道路等。
    语义分割处理stuff(也把things当stuff来分割),它将图像中每个像素进行分类。
    实例分割处理things,它检测出图像中的目标,并分割出实例。
    而全景分割既分割things,也分割stuff。
